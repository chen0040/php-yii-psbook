...
?>
	...
	<tr>
	<td colspan="2">
	<div id="<?php echo $p->tag_lcase().'_conversation'; ?>"></div>
	<?php 
		$cs=Yii::app()->getClientScript();
		$cs->registerCss(
		'css-message-'.$p->tag_lcase().'-conversation',
		'
		div#'.$p->tag_lcase().'_conversation{
		  height: 300px;
		  display:block;
		  overflow: auto;
		}
		...
		');
		//echo CHtml::textArea($p->tag_lcase().'_conversation', '', array('rows'=>12, 'cols'=>100)); 
	?>
	</td>
	</tr>
	...
	<tr>
	<td colspan="2">
	<?php echo CHtml::textArea($p->tag_lcase().'_message_body', '', array('rows'=>4, 'cols'=>88)); ?>
	</td>
	</tr>
	
	 <tr>
	 <td width="300px">
		<div id="btn-message-<?php echo $p->tag_lcase(); ?>-clear-history-complete" class="gray">
        ...
        <?php 
			$cs=Yii::app()->getClientScript();
			$cs->registerScript(
				'handle-message-'.$p->tag_lcase().'-clear-history-complete',
				'
				function handleMessage'.$p->tag_ucase().'ClearHistoryComplete()
				{
					$.post("index.php?r='.$p->tag_lcase().'/clearMessages&id='.$p->id.'", { stype:"professor", tid:'.$model->id.'},
					function(data) 
					{
						...
						else
						{
							...
							{
								if(data.messages[i].from=="'.$p->id.'" && data.messages[i].from_type=="'.$p->mtype().'")
								{
							...
							$("#'.$p->tag_lcase().'_conversation").html(result);
						}
				   },
				   "json");
				}
			...
			$cs->registerScript(
				'register-message-'.$p->tag_lcase().'-clear-history-complete',
				'
				$("#btn-message-'.$p->tag_lcase().'-clear-history-complete").click(function() {
					handleMessage'.$p->tag_ucase().'ClearHistoryComplete();
				});
				',
	 ...
	 <td width="300px">
		<div id="btn-message-<?php echo $p->tag_lcase(); ?>-complete" class="gray">
		  Send
		</div>
        ...
        <?php 
			$cs=Yii::app()->getClientScript();
			$cs->registerScript(
				'handle-message-'.$p->tag_lcase().'-complete',
				'
				function handleMessage'.$p->tag_ucase().'Complete()
				{
					$.post("index.php?r='.$p->tag_lcase().'/message&id='.$p->id.'", { stype:"'.$model->tag_lcase().'", tid:'.$model->id.', message_body:$("#'.$p->tag_lcase().'_message_body").val()},
					function(data) 
					{
						if(data.status=="failure")
						{
			...
			$cs->registerScript(
				'register-message-'.$p->tag_lcase().'-complete',
				'
				$("#btn-message-'.$p->tag_lcase().'-complete").click(function() {
					handleMessage'.$p->tag_ucase().'Complete();
				});
				',
				CClientScript::POS_READY
			);
		...
		<tr>
		<td>
		<div id="btn-message-<?php echo $p->tag_lcase(); ?>-refresh-history-complete" class="gray">
		  Refresh History
		</div>
		<?php 
			$cs=Yii::app()->getClientScript();
			$cs->registerScript(
				'handle-message-'.$p->tag_lcase().'-refresh-history-complete',
				'
				function handleMessage'.$p->tag_ucase().'RefreshHistoryComplete()
				{
					$.post("index.php?r='.$p->tag_lcase().'/showMessages&id='.$p->id.'", { stype:"professor", tid:'.$model->id.'},
					function(data) 
					{
						if(data.status=="failure")
						{
			...
			$cs->registerScript(
				'register-message-'.$p->tag_lcase().'-refresh-history-complete',
				'
				$("#btn-message-'.$p->tag_lcase().'-refresh-history-complete").click(function() {
					handleMessage'.$p->tag_ucase().'RefreshHistoryComplete();
				});
				',
				CClientScript::POS_READY
			);
	...
	<td>
		...
		?>
		<div id="btn-message-<?php echo $p->tag_lcase(); ?>-cancel" class="gray">
		  Close
		</div>
        <?php 
			$cs=Yii::app()->getClientScript();
			...
			$cs->registerScript(
				'handle-message-'.$p->tag_lcase().'-cancel',
				'
				function handleMessage'.$p->tag_ucase().'Cancel()
				{
					$("#message-'.$p->tag_lcase().'-dialog").dialog("close");
				}
				',
				CClientScript::POS_END
			);
			$messages=$p->getMessagesToProfessor($model->id);
			$chtml='';
			$msg_id='';
			foreach($messages as $msg)
			{
				...
				if($msg->from_id==$p->id && $msg->from_type==$p->mtype())
				{
			...
			$chtml.='';
			$cs->registerScript(
				'register-message-'.$p->tag_lcase().'-cancel',
				'
				$("#btn-message-'.$p->tag_lcase().'-cancel").click(function() {
					handleMessage'.$p->tag_ucase().'Cancel();
				});
				$("#'.$p->tag_lcase().'_conversation").html("'.$chtml.'");
				$("#message_id").val("'.$msg_id.'");
				$("div.gray").corner();
				$("div.black").corner();
				window.setInterval(handleMessage'.$p->tag_ucase().'RefreshHistoryComplete, 10000);
				',
